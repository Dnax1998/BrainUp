...
# --- KONFIGURACJA AGRESYWNA (Z v8.6) ---
TRADE_AMOUNT_USDC = 250.0      # Zwiększona kwota zakupu dla większego zysku
STOP_LOSS_PCT = 0.04           # Stop Loss na poziomie 4%
TRAILING_ACTIVATE_PCT = 0.025  # Śledzenie zysku od 2.5%
RSI_BUY_THRESHOLD = 50         # Kupuj częściej (RSI < 50)
RSI_SELL_THRESHOLD = 65        # Sprzedawaj przy RSI > 65
...
display_state = {
    ...
    "last_action": "System v8.6 Aggressive Gotowy...",
    ...
    try:
        ...
        for symbol in ["BTC", "ETH"]:
            ...
            if amt > 0.0001 and entry_price > 0:
                ...
                if price_change <= -STOP_LOSS_PCT or rsi_val > RSI_SELL_THRESHOLD:
                    mexc.create_order(pair, 'limit', 'sell', amt, round(price * 0.999, 2))
                    display_state["sell_count"] += 1
                    entry_price = 0
                    ai_thoughts.append(f"SELL {symbol}")
                elif price_change >= TRAILING_ACTIVATE_PCT and rsi_val < 55:
                    mexc.create_order(pair, 'limit', 'sell', amt, round(price * 0.999, 2))
                    display_state["sell_count"] += 1
                    entry_price = 0
                    ai_thoughts.append(f"TRAIL {symbol}")
            ...
            if usdc_free >= TRADE_AMOUNT_USDC and rsi_val < RSI_BUY_THRESHOLD:
                ...
                entry_price = ((amt * entry_price) + (buy_qty * price)) / (amt + buy_qty) if amt > 0 else price
                display_state["buy_count"] += 1
                ai_thoughts.append(f"BUY {symbol}")
        ...
        display_state.update({
            ...
            "profit": round(current_total_value - INITIAL_CAPITAL, 2),
            "last_action": " | ".join(ai_thoughts) if ai_thoughts else f"Analiza: BTC:{assets_update['BTC']['rsi']} ETH:{assets_update['ETH']['rsi']}",
...
def home():
    ...
    <html>
    <head>
        <title>AI TRADER v8.6 AGGRESSIVE</title>
    ...
    <body>
        <h3 style="color: #f3ba2f; text-align:center; margin-bottom: 15px;">🚀 AI TRADER v8.6 AGGRESSIVE</h3>
        <div class="grid">
