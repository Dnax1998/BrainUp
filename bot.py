...
display_state = {
    ...
    "last_action": "System BrainUp v9.3 Online", # ZŁOTY STATUS AI
...
def run_loop():
    ...
    try:
        ...
        assets_update = {}
        actions = [] # Kolekcjoner akcji dla złotego panelu
        ...
        for symbol in ["BTC", "ETH"]:
            ...
            entry = 0 # Uproszczone entry, synchronizuj z bazą w produkcji

            if amt > 0.00001:
                # Logika sprzedaży (uproszczona, dodaj entry_price dla StopLoss)
            ...
            if usdc_free >= TRADE_AMOUNT_USDC and rsi_val < RSI_BUY_THRESHOLD and amt < 0.00001:
                if ask_ai_decision(symbol, price, rsi_val):
                    ...
                    actions.append(f"AI WAIT {symbol} (RSI {rsi_val})")

            assets_update[symbol] = {"amount": round(amt, 6), "rsi": rsi_val}

        # Aktualizacja głównego statusu AI
        ai_status = " | ".join(actions) if actions else f"Mózg Llama 3: Skanowanie trendów BTC/ETH..."

        display_state.update({
            "usdc": round(usdc_free, 2), "total": round(current_total, 2), 
            "profit": round(current_total - INITIAL_CAPITAL, 2),
            "last_action": ai_status, # ZŁOTY STATUS AI
        ...
        display_state["last_action"] = f"Błąd Krytyczny: {str(e)}"
...
def get_data(range_type):
    ...
    if range_type == 'day':
        for i in range(23, -1, -1):
            ...
            match = get_closest(target, 3500)
            if match: points.append({"t": target.strftime("%H:00"), "v": match['v']})
            
    # Dodaj logikę dla week/month identyczną jak w v9.2...
...
def home():
    ...
    <!DOCTYPE html><html><head><title>BrainUp v9.3 Platinum</title>
    ...
    <style>
        ...
        /* PRZYWRÓCONY ZŁOTY PANEL STATUSU AI */
    ...
    <body>
        ...
        <h3 style="color: #f3ba2f; text-align:center;">🧠 AI TRADER v9.3 PLATINUM</h3>
        ...
        <div class="grid">
            ...
            <div class="card">
                ...
                <div class="sub-label">Sprzedaże: <b id="s_count" style="color:white;">0</b></div>
            </div>
            <div class="card">
                <div class="label">Wartość Portfela</div>
                <div id="total" class="value">--</div>
                <div class="sub-label">Kupna: <b id="b_count" style="color:white;">0</b></div>
            </div>
        </div>

        <div class="ai-box"><b>Llama 3 Analytics:</b> <span id="ai_action">Inicjalizacja mózgu...</span></div>
